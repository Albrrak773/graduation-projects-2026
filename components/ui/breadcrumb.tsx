...
function BreadcrumbList({ className, ...props }: React.ComponentProps<"ol">) {
  return (
    <ol
      ...
      className={cn(
        "flex flex-wrap items-center gap-1.5 text-base/relaxed wrap-break-word text-muted-foreground",
        className
      )}
      {...props}
    />
...
function BreadcrumbSeparator({ children, className, ...props }: React.ComponentProps<"li">) {
  return (
    <li
      ...
      role="presentation"
      aria-hidden="true"
      className={cn("text-muted-foreground/40 select-none", className)}
      {...props}
    >
      {children ?? <span>/</span>}
    </li>
